...
class Slide
{
	...
	{
		// 	 Récupérer le slide $id avec se traduction $locale en base de données
	...
	public function querySlide($id, $locale)
	{
		...
		// Faire la requete
		$query =$pdo->prepare('SELECT *  FROM slides JOIN slide_translations  ON slides.id = slide_translations.slides_id AND slide_translations.locale = ? WHERE slides.id= ? ');
